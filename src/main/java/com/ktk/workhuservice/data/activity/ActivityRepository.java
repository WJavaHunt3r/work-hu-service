...
public interface ActivityRepository extends JpaRepository<Activity, Long> {
    ...
    @Query("SELECT ac FROM Activity ac WHERE (ac.responsible.id = ?1 OR ?1 IS NULL) " +
            ...
            " AND (lower(ac.description) like lower(concat('%', concat(?6, '%')))  OR ?6 IS NULL)")
    List<Activity> fetchByQuery(Long responsible, Long employer, Boolean registeredInApp, Boolean registeredInMyShare, Long createUser, String searchText);
}
